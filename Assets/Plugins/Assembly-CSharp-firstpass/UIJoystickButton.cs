...
public class UIJoystickButton : UIButtonBase
{
	...
	public override bool HandleInput(UITouchInner touch)
	{
		...
		if (touch.fingerId == m_FingerId)
		{
			...
			{
				float num3 = touch.position.x - m_Center.x;
				float num4 = touch.position.y - m_Center.y;
				...
				if (num4 >= 0f)
				{
					m_Direction = Mathf.Atan2(num4, num3);
				}
				else
				{
					m_Direction = Mathf.Atan2(num4, num3) + (float)Math.PI * 2f;
				}
				UpdatePosition();
				m_Parent.SendEvent(this, 1, m_Direction, m_Distance);
			}
