...
public class XXTEAUtils
{
	...
	{
		byte[] array = Decrypt(Convert.FromBase64String(text), Encoding.UTF8.GetBytes(key));
		if (array == null)
		{
			return null;
		}
	...
	protected static byte[] Encrypt(byte[] Data, byte[] Key)
	{
		if (Data.Length == 0)
		{
			return null;
	...
	protected static byte[] Decrypt(byte[] Data, byte[] Key)
	{
		if (Data.Length % 4 != 0)
		{
			return null;
